...
that has Pillow when conversion flags are used. `<repo>` is the comic-ai-tools
checkout.
...
| a folder that contains one sub-folder per chapter/issue | none — each sub-folder becomes its own `.cbr` inside the folder |
| a folder that contains the images directly | none — one `.cbr` next to the folder, named after it |
...
| The user says... | Flags |
| --- | --- |
| "extract / unpack these archives" | none — each archive becomes a sibling folder with the same name (`Comic (1)` if it already exists) |
| "just show me what would happen", "preview" | `--dry-run` |
| "get the covers", "first page of each", "thumbnails" | `--first-only` (writes `<archive stem>.<ext>` next to each archive, nothing else) |
...
```bash
<repo>/venv/bin/python comic-archive/scripts/images_to_cbr.py "/path/to/comics"                       # one .cbr per sub-folder
