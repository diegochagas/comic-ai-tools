...
def default_tessdata_dir() -> Path | None:
    local = Path(__file__).resolve().parent.parent / "tessdata"   # <skill>/tessdata
    if local.exists():
        return local
    return None
...
def choose_ocr_language(tesseract: str, preferred: str, tessdata_dir: Path | None) -> str:
    ...
    print("       Run setup.sh to download them into japanese-ocr-translate/tessdata, or pass --tessdata-dir.")
    sys.exit(1)
