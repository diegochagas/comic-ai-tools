...
multiple comic projects (folders under `projects/` at the repo root). No LLM
API is used — you (the current agent session: Claude Code, Codex, ...) do the
orchestration and visual QC yourself, and Higgsfield does the image generation through its CLI, billed
in credits to Diego's Higgsfield Plus plan (1000 credits/month).

## Paths

All commands run from the repo root (the folder holding `setup.sh`):
projects live in `projects/<name>/`, the pipeline scripts in
`gerar-paginas/scripts/`, the new-project template in `gerar-paginas/_template/`.
...
`python3 gerar-paginas/scripts/make_lettering_guide.py -p <project> <issue>` — it writes
...
- After each batch of 5: run `python3 gerar-paginas/scripts/status.py -p <project>` and
  report approved / pending / needs_review, plus anything flagged.
- When all pages of an issue are approved:
  `python3 gerar-paginas/scripts/assemble_cbz.py -p <project> <issue>` and send Diego the
  .cbz path.
