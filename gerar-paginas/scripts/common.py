...
SKILL = Path(__file__).resolve().parent.parent   # gerar-paginas/
ROOT = SKILL.parent                                # repo root
PROJECTS = SKILL / "projects"                     # gerar-paginas/projects/<name>/
...
def resolve_project(arg: str | None) -> tuple[str, dict, Path]:
    ...
    names = list_projects()
    if not names:
        sys.exit("No projects found under gerar-paginas/projects/ (need a project.json)")
    if arg is None:
        if len(names) == 1:
            arg = names[0]
        else:
