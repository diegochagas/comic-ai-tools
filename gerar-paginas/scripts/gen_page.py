...
"""
import sys, os, json, subprocess, urllib.request, tempfile, shutil

SKILL = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))  # gerar-paginas/ (projects/ lives there)
HF = shutil.which('higgsfield') or os.path.expanduser('~/hf/node_modules/.bin/higgsfield')
...
def refs(project, job):
    base = os.path.join(SKILL, 'projects', project, 'refs')
    out = []
...
def main():
    ...
    job = json.load(open(os.path.join(SKILL, 'projects', project, 'jobs', issue, f'page_{page:02d}.json')))
    ...
    gendir = os.path.join(SKILL, 'projects', project, 'work', issue, 'gen')
